...
def argument_parser():
    ...
    parser.add_argument('--hidden_dim', help = 'hidden dimension for node features', default = 32, type = int)
    ...
    parser.add_argument('--dropout', help = 'dropout beteen the hidden layers', default = 0.50, type = float)
    parser.add_argument('--wdecay', help = 'weight decay', default = 0.0005, type = float)
...
weight_decay = parsed_args.wdecay
...
if dataset == 'cora':
    ...
    data_obj = Cora()
    # print("Train ", len(data_obj.train_mask))
    # print("Val ", len(data_obj.val_mask))
    # print("Test ", len(data_obj.test_mask))
...
model = RPE_GNN(data_obj, num_layers = hidden_layers, hidden_dim = hidden_dim, dropout = dropout, device = device)
...
for i in range(test_iter):
    ...
    if acc > max_acc:
        max_acc = acc
    avg_acc += acc
    print(f"Test iteration: {i+1: 04d}, complete --- accuracy: {acc: .4f}")
